...
public class VT_window : EditorWindow {
	...
	[SerializeField]
	private string[] combinedSize = new string[7] {"64", "128", "256", "512", "1024", "2048", "4096"};
	[SerializeField]
    ...
    */
	...
	void guiForVersion() {
		...
		GUILayout.Box ("Version 2.1.2", bodyH1Style, GUILayout.Height(20), GUILayout.ExpandWidth(true));
		...
		for (int g = 0; g < currentGameObjects.Length; g++) {
			...
			for (int i = 0; i < currentVertices[g].Length; i++) {
				...
				if (showVertexIndicators) {
					...
					Handles.SphereHandleCap (0, vertPos, Quaternion.identity, vertexIndicatorSize * falloff, EventType.Repaint);
